...
function MeetingTile({ meeting, isEnabled, onToggle }: { meeting: MeetingTileInfo, isEnabled?: boolean, onToggle?: (isEnabled: boolean) => void }) {
  const cardContent = (
...
export function DashboardClient({ accountsWithEvents, pastMeetings, initialEnabledIds }: {
  ...
  return (
    <Tabs defaultValue="upcoming">
      ...
      {/* Past Meetings tab remains the same */}
      <TabsContent value="past">
        {pastMeetings.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {pastMeetings.map(meeting => <MeetingTile key={meeting.id} meeting={meeting} />)}
          </div>
        ) : (
          <div className="text-center py-12 border-2 border-dashed rounded-lg">
